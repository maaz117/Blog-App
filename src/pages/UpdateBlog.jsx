...
import updateBlogs from "../hooks/updateBlogs";
import getBlogById from "../hooks/getBlogById";

const UpdateBlog = () => {
  const { id } = useParams(); 
  const navigate = useNavigate();

  const { loading, error, updateBlogData } = updateBlogs();

  const { blog, fetchBlogById } = getBlogById();
  ...
  const formik = useFormik({
    initialValues: blog || { title: "", content: "", author: "", category: "" },
    validationSchema: blogValidationSchema,
    enableReinitialize: true, 
    onSubmit: async (values) => {
      try {
        await updateBlogData(id, values);
        navigate("/"); 
      } catch (error) {
